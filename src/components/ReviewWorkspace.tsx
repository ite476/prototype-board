import { proposalsFor } from '../core/board.ts';
import type { BoardData, Idea, Proposal, ReviewCommand } from '../core/model.ts';
...
export function ReviewWorkspace({ data, idea, proposalId, registry, saving, onSelect, onSave }: {
  ...
  onSelect: (proposal: Proposal) => void; onSave: (command: ReviewCommand) => Promise<void>;
}) {
  ...
  return <main className="board-main">
    ...
    <WorkflowPanel data={data} idea={idea} />
    <div className="review-layout">
      <aside className="option-panel" aria-label="시안 선택">
        ...
        {proposal && <div className="review-actions">
          <button className="primary-button" disabled={saving || idea.selectedProposalId === proposal.id} onClick={() => void onSave({ type: 'decision', ideaId: idea.id, proposalId: proposal.id })}>
            {idea.selectedProposalId === proposal.id ? '채택한 시안' : '이 시안 채택'}
          </button>
          {idea.selectedProposalId && <button className="text-button" disabled={saving} onClick={() => void onSave({ type: 'decision', ideaId: idea.id })}>채택을 취소하고 다시 검토</button>}

...
소스 코드는 [MIT 라이선스](LICENSE)로 사용할 수 있습니다. 현재는 npm 패키지로 배포하지 않으며, `package.json`의 `private` 설정으로 실수로 npm에 게시되지 않게 했습니다.

프로그램을 fork하더라도 실제 작업 자료와 비밀 설정은 별도 보관해야 합니다. 이 저장소의 공개 여부와 실행 중인 보드 화면의 접근 권한은 서로 다릅니다. 현재 보드는 인증·사용자 권한이 없는 로컬 단일 사용자용 초안이므로, 인터넷에 직접 공개 서비스로 노출하지 마세요.
